...
static inline uint64_t sym_profile_sha3_begin(void) {
    return k_cycle_get_64();
}

static inline void sym_profile_sha3_end(uint64_t start_cycles) {
    symmetric_profile_add_sha3_keccak(k_cycle_get_64() - start_cycles);
}
#else
static inline uint64_t sym_profile_sha3_begin(void) {
    return 0;
...
static void KeccakF1600_StatePermute(uint64_t *state) {
    uint64_t profile_start_cycles = sym_profile_sha3_begin();
    int round;
    ...
    state[23] = Aso;
    state[24] = Asu;
    sym_profile_sha3_end(profile_start_cycles);
}
