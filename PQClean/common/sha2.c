...
static inline uint64_t sym_profile_sha2_begin(void) {
    return k_cycle_get_64();
}

static inline void sym_profile_sha2_end(uint64_t start_cycles) {
    symmetric_profile_add_sha2(k_cycle_get_64() - start_cycles);
}
#else
static inline uint64_t sym_profile_sha2_begin(void) {
    return 0;
...
static size_t crypto_hashblocks_sha256(uint8_t *statebytes,
    ...
    uint64_t profile_start_cycles = sym_profile_sha2_begin();
    uint32_t state[8];
    uint32_t a;
    uint32_t b;
    uint32_t c;
    ...
    sym_profile_sha2_end(profile_start_cycles);
    return inlen;
}
